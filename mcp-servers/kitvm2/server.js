#!/usr/bin/env node
/**
 * KITVM2 MCP server — exposes Hyper-V VM management on KITVM2 (Dell
 * PowerEdge R810, Windows Server 2022, 64 logical CPUs, 192 GB RAM) as tools
 * for Claude. All PowerShell cmdlets run over SSH via Tailscale: Claude
 * container → Tailscale mesh → KITVM2 OpenSSH Server → PowerShell.
 *
 * SSH config: relies on a "Host kitvm2" block in /home/node/.ssh/config.
 * No Cloudflare Tunnel cold-start delay — Tailscale connections are fast.
 *
...
const SSH_HOST = process.env.KITVM2_SSH_HOST || 'kitvm2';
const SSH_TIMEOUT_DEFAULT = parseInt(process.env.KITVM2_SSH_TIMEOUT || '60', 10);

const server = new Server(
  { name: 'lab-kitvm2', version: '0.1.0' },
  { capabilities: { tools: {} } },
);
...
const tools = [
  {
    name: 'kitvm2_get_host_info',
    description: 'Return KITVM2 Hyper-V host details: hostname, OS, total memory, logical processor count, virtualization extensions, etc.',
    inputSchema: { type: 'object', properties: {} },
  },
  {
    name: 'kitvm2_list_vms',
    description: 'List all Hyper-V VMs on KITVM2 with name, state, assigned memory, vCPU count, uptime, and IP addresses (if Integration Services are reporting).',
    inputSchema: { type: 'object', properties: {} },
  },
  {
    name: 'kitvm2_get_vm',
    description: 'Get full detail on a single VM on KITVM2 including network adapters, integration services, and configured generation.',
    inputSchema: {
      type: 'object',
      required: ['name'],
  ...
  {
    name: 'kitvm2_list_vswitches',
    description: 'List Hyper-V virtual switches on KITVM2. Use the Name field when calling kitvm2_create_vm.',
    inputSchema: { type: 'object', properties: {} },
  },
  {
    name: 'kitvm2_list_checkpoints',
    description: 'List checkpoints (snapshots) for a VM on KITVM2, in chronological order.',
    inputSchema: {
      type: 'object',
      required: ['vm_name'],
  ...
  {
    name: 'kitvm2_start_vm',
    description: 'Power on a VM on KITVM2 (Start-VM). No-op if already running.',
    inputSchema: {
      type: 'object',
      required: ['name'],
  ...
  {
    name: 'kitvm2_stop_vm',
    description: 'Gracefully shut down a VM on KITVM2 via Hyper-V Integration Services (Stop-VM).',
    inputSchema: {
      type: 'object',
      required: ['name'],
      properties: { name: { type: 'string' } },
  ...
  {
    name: 'kitvm2_turn_off_vm',
    description: '⚠ Hard power-cut a VM on KITVM2 (Stop-VM -TurnOff). Equivalent to pulling the plug. Use only when graceful stop fails.',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'kitvm2_save_vm',
    description: 'Suspend a VM to disk on KITVM2 (Save-VM). Resume with kitvm2_start_vm.',
    inputSchema: {
      type: 'object',
      required: ['name'],
      properties: { name: { type: 'string' } },
  ...
  {
    name: 'kitvm2_pause_vm',
    description: 'Pause a VM on KITVM2 (Suspend-VM). CPU is halted; use kitvm2_resume_vm to continue.',
    inputSchema: {
      type: 'object',
      required: ['name'],
      properties: { name: { type: 'string' } },
  ...
  {
    name: 'kitvm2_resume_vm',
    description: 'Resume a paused VM on KITVM2 (Resume-VM).',
    inputSchema: {
      type: 'object',
      required: ['name'],
      properties: { name: { type: 'string' } },
  ...
  {
    name: 'kitvm2_restart_vm',
    description: 'Restart a VM on KITVM2 (Restart-VM). Graceful by default.',
    inputSchema: {
      type: 'object',
      required: ['name'],
      properties: {
  ...
  {
    name: 'kitvm2_checkpoint_vm',
    description: 'Take a Hyper-V checkpoint (snapshot) of a VM on KITVM2.',
    inputSchema: {
      type: 'object',
      required: ['vm_name'],
      properties: {
  ...
  {
    name: 'kitvm2_restore_checkpoint',
    description: '⚠ Restore a VM on KITVM2 to a previous checkpoint. Discards state since the checkpoint. Always confirm with the user.',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'kitvm2_remove_checkpoint',
    description: 'Delete a checkpoint on KITVM2. Hyper-V merges the differencing disk back into the parent VHD.',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'kitvm2_create_vm',
    description: 'Create a new Hyper-V VM on KITVM2. Discover valid switch names with kitvm2_list_vswitches first.',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'kitvm2_destroy_vm',
    description: '⚠ PERMANENTLY destroy a VM on KITVM2 (Remove-VM). Irreversible. Always confirm with the user.',
    inputSchema: {
      type: 'object',
      required: ['name', 'confirm'],
      properties: {
  ...
  {
    name: 'kitvm2_rename_vm',
    description: 'Rename a VM on KITVM2 (Rename-VM). Underlying VHD filenames are NOT renamed.',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'kitvm2_run_in_vm',
    description: 'Run a PowerShell script inside a guest VM on KITVM2 via Hyper-V Integration Services (Invoke-Command -VMName).',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'kitvm2_run_powershell',
    description: 'Run an arbitrary PowerShell command on KITVM2 (NOT inside a guest). Escape hatch when no specific tool fits.',
    inputSchema: {
      type: 'object',
      required: ['command'],
      properties: {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  switch (name) {
    ...
    case 'kitvm2_restore_checkpoint':
